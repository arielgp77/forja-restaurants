...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.redirect(new URL(`/orders/${orderId}?updated=1`, request.url));
  } catch (error) {
    const msg = error instanceof Error ? error.message : "update_failed";
    return NextResponse.redirect(new URL(buildRedirect(`/orders/${orderId}`, msg), request.url));
  }
}